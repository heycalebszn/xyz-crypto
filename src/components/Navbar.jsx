...
const Navbar = () => {
  ...
  return (
    <div className="flex justify-between shadow-xl h-[10vh] fixed w-full items-center">
        <h1 className="text-white font-semibold ml-[50px]">XYZ</h1>
        ...
        <ul className="sm:flex block text-center justify-center mr-[20px]">
            {Navlinks.map((item) => (
                <li key={item.id}><a className="text-white sm:pr-[10px] pr-0 sm:text-[10px] text-[12px] w-full" href="">{item.label}</a></li>
            ))}
            </ul>
            ...
            {openNavigation ? (
                <XIcon className="cursor-pointer text-white sm:hidden mr-[50px]" onClick={handleNavigationToggle} />
            ) : (
                <MenuIcon className="cursor-pointer text-white sm:hidden mr-[50px]" onClick={handleNavigationToggle} />
            )
            }
    </div>
  )
