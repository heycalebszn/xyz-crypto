...
const Navbar = () => {
  ...
  return (
        ...
        <div className={`md:flex justify-center sm:transition-none md:border-none items-center md:bg-none bg-white absolute right-[10px] top-[80px] border border-white transition duration-300 p-[20px] md:p-0 rounded-lg w-[40vw] sm:translate-y-[0] md:right-0 shrink-0 md:static
        ${openNavigation ? "sm:translate-y-[0]" : "translate-y-[-2500px]"}`}>
        <ul className="sm:flex shrink-0 block text-center justify-center mr-[20px]">
            {Navlinks.map((item) => (
                <li key={item.id}><a className="text-black sm:pr-[10px] pr-0 sm:text-[12px] text-[12px] w-full font-semibold" href="">{item.label}</a></li>
            ))}
            </ul>
            ...
            <div className="bg-red-500 shrink-0 rounded-md px-[15px] py-[10px] hover:bg-[--body-color] hover:border border-white transition duration-300 cursor-pointer relative text-center items-center justify-center sm:mt-0 mt-[20px] mr-[50px] hidden md:flex">
            <a href="#" className="text-white font-semibold text-[12px]">Get Started</a>
            <ArrowRight className="text-white pl-[5px] w-[20px]" />
            </div>
